...
public class Fizzy : Body
{
    ...
    public override void AbilityUpdate(ref Vector2 playerVelo, Vector2 moveSpeed)
    {
        ...
        if(OnSkateboard)
        {
            ...
            if (trail > 0)
            {
                starTimer -= Player.PassiveAttackSpeedModifier * Time.fixedDeltaTime;
                while (starTimer <= 0)
                {
                    ...
                    Projectile.NewProjectile<StarProj>((Vector2)Skateboard.transform.position + circular * 2 + new Vector2(0, 0.3f), circular * 8, 2, Player, randPos.x, randPos.y, Utils.RandFloat(-1, 1));
                }
            }
        }
    }
    public void SwapSkateboard()
    {
        ...
        if(SkateboardMountCounter >= SkateboardMountTime || SkateboardMountCounter <= 0)
        {
            ...
            if(OnSkateboard)
            {
                ...
                for(int i = 0; i < c; ++i)
                    Projectile.NewProjectile<SkateboardProj>(Skateboard.transform.position, Player.RB.velocity, 8 + Player.SkateboardBonusDamage, Player, Utils.RandFloat(-1, 1) * (0.5f + i * 0.1f));
